...
namespace Dash.Engine
{
    ...
    public struct AxisAlignedBoundingBox
    {
        ...
        public Vector3 this[int index]
        {
            get
            {
                if (index == 0) return new Vector3(Max.X, Max.Y, Max.Z);
                if (index == 1) return new Vector3(Max.X, Max.Y, Min.Z);
                if (index == 2) return new Vector3(Max.X, Min.Y, Min.Z);
                if (index == 3) return new Vector3(Min.X, Max.Y, Min.Z);

                if (index == 4) return new Vector3(Min.X, Min.Y, Min.Z);
                if (index == 5) return new Vector3(Min.X, Min.Y, Max.Z);
                if (index == 6) return new Vector3(Min.X, Max.Y, Max.Z);
                if (index == 7) return new Vector3(Max.X, Min.Y, Max.Z);
        ...
        /// <param name="b">AxisAlignedBoundingBox to test in this AxisAlignedBoundingBox</param>
        /// <returns>True if the AxisAlignedBoundingBox intersects this AxisAlignedBoundingBox</returns>
        public bool IntersectsExclusive(AxisAlignedBoundingBox b)
        {
            // Test the six planes of the box
            if (max.X < b.Min.X) return false;
            if (max.Y < b.Min.Y) return false;
        ...
        public static AxisAlignedBoundingBox FromCenterSize(Vector3 center, Vector3 size)
        {
            Vector3 halfSize = size / 2f;
            return new AxisAlignedBoundingBox(center - halfSize, center + size);
        }
        #endregion
    }
